...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for char in range(0, len(lowerList)):
            if (char + shift) >= len(lowerList):
                self.shiftDict[lower[char]] = lower[abs(char - (len(lowerList) - shift))]
                self.shiftDict[upper[char]] = upper[abs(char - (len(upperList) - shift))]
            else:
                self.shiftDict[lower[char]] = lower[char + shift]
                self.shiftDict[upper[char]] = upper[char + shift]
